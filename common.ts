...
import { readableStreamToString, ClickFunnelsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ClickFunnelsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ClickFunnelsError(e, e.response?.data)
                }
                if (innerError instanceof ClickFunnelsError) {
                    // Got "ClickFunnelsError" from the above try block
                    throw innerError;
                }
